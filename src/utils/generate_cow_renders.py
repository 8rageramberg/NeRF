...
# create the default data directory under the project root
ROOT_DIR = Path(__file__).resolve().parents[2]
DATA_DIR = ROOT_DIR / "data" / "cow_mesh"


def _ensure_cow_mesh(data_dir: Path) -> None:
    ...
    for filename, url in mesh_sources.items():
        ...
        try:
            ...
            raise RuntimeError(
                "Failed to download cow mesh assets. Download them manually "
                "from https://dl.fbaipublicfiles.com/pytorch3d/data/cow_mesh/"
                " and place them in data/cow_mesh/."
            ) from err


def generate_cow_renders(
    num_views: int = 40, data_dir: Path | str = DATA_DIR, azimuth_range: float = 180
):
    """
    ...
    mesh = load_objs_as_meshes([str(obj_filename)], device=device)
